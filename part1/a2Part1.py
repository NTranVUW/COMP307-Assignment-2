...
if __name__ == '__main__':
    ...
    nn = Neural_Network(n_in, n_hidden, n_out, initial_hidden_layer_weights, initial_output_layer_weights, initial_hidden_layer_biases, initial_output_layer_biases, learning_rate)
    ...
    hidden_layer_weights, output_layer_weights = nn.forward_pass(instances[0])
    delta_output_layer_weights, delta_hidden_layer_weights, delta_output_layer_biases, delta_hidden_layer_biases = nn.backward_propagate_error([instances[0]], hidden_layer_weights, output_layer_weights, onehot_encoded[0])

    nn.update_weights(delta_output_layer_weights, delta_hidden_layer_weights, delta_output_layer_biases, delta_hidden_layer_biases)

    print('Weights after performing BP for first instance only:')
    print('Hidden layer weights:\n', nn.hidden_layer_weights)
    print('Output layer weights:\n', nn.output_layer_weights)
    print('Hidden layer biases:\n', nn.hidden_layer_biases)
    print('Output layer biases:\n', nn.output_layer_biases)
    ...
    # plt.plot(nn.convergence)
    # plt.ylim(1, 0.4)
    # plt.title('Convergence of training with 2 hidden nodes')
    # plt.xlabel('Epochs')
    # plt.ylabel('MSE')
    # plt.show()
    ...
    predictions = np.transpose(np.array(nn.predict(test_instances), ndmin=2))
    label_encoder, integer_encoded, onehot_encoder, onehot_encoded = encode_labels(test_labels)

    correct = np.sum((integer_encoded == predictions))
    test_acc = correct / len(predictions)
    print('\ntest acc: ', correct, '/', len(predictions), '=', round(test_acc * 100, 2), '%')
